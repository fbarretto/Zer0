...
Pulso::Pulso(ofPoint _position, float _speed, float _radiusLimit){
    radiusOffset = 10; //initial offset for radius is set here
    radiusLimit = 100; //initial limit of radius is set here
    position = _position;
    speed = _speed;
...
void Pulso::draw(){
    if(isActive) {
        ...
        ofSetColor(255,255, 255,int(255*(1-radius/radiusLimit))); //sets the color to white with alfa relative to radius to radiusLimit ratio
        ofNoFill();
